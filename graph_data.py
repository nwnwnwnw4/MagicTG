...
def result_data():
    ...
    f.close()
    list_result = [[ 'W', 'U', 'B', 'R', 'G']]
    a = []

    for key in result.keys():
        a.append([key,sum(result[key]), result[key].index(max(result[key]))])
    ...
    for i in range(15):
        for j in range(5):
            key = b[j+1][i][0]
            list_result.append((key, [result[key][1:]]))
    return list_result
...
if __name__ == '__main__':
    ...
    color = {0:'y', 1:'b', 2:'k', 3:'r', 4:'g'}
    for ax, (title, case_data) in zip(axes.flatten(), data):
        # ax.set_rgrids([1, 10, 100, 1000])
        ax.set_title(title, weight='bold', size='medium', position=(0.5, 1.1),
                     horizontalalignment='center', verticalalignment='center')
        d = case_data[0]
        color_index = d.index(max(d))
        ax.plot(theta, d, color=color[color_index])
        ax.fill(theta, d, facecolor=color[color_index], alpha=0.25)
    ...
    fig.text(0.5, 0.965, 'Most Frequently Used Keywords in Magic by Color',
